...
from src.database.db import get_attedance_for_teacher
...
def teacher_tab_take_attendance():
    ...
    with col2:
        if st.button("Add photos",type="primary",icon=":material/photo_prints:",width="stretch"):
            add_photos_dialog()
    select_subject_id = subject_options[selected_subject_label]
    st.divider()
    ...
    with c2:
        if st.button("Run Face Analysis",width="stretch",type="secondary",icon=":material/analytics:",disabled=not has_photos):
            with st.spinner("Deep scanning classroom photos.."):
                ...
                enrolled_response = supabase.table("subject_students").select("*,students(*)").eq("subject_id",select_subject_id).execute()
...
def teacher_tab_manager_subjects():
    teacher_id = st.session_state.teacher_data["teacher_id"]
    ...
    if subjects:
        ...
        def share_btn():
            if st.button(f"Share subject code: {sub['name']}",key=f"share_{sub['subject_code']}",icon=":material/share:"):
...
def teacher_tab_attendance_records():
    ...
    teacher_id = st.session_state.teacher_data["teacher_id"]
    records = get_attedance_for_teacher(teacher_id)
    if not records:
        return
    data = []
    for r in records:
        ...
        data.append({
            ...
            "Time": datetime.datetime.fromisoformat(ts).strftime("%Y-%m-%d %I:%M %p") if ts else "NA",
    ...
    summary = (
        df.groupby(["ts_group","Time","Subject","Subject code"]).agg(Present_Count=("is_present","sum"),Total_Count=("is_present","count")).reset_index()
    )
...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout",type="secondary",key="loginbackbtn",shortcut="control+backspace"):
            st.session_state['is_looged_in'] = False
...
def teacher_screen_register():
    ...
    teacher_password = st.text_input("Enter password",type="password")
    teacher_password_confirm = st.text_input("Confirm your password",type="password")
    st.divider()
    btnc1,btnc2 = st.columns(2)
    with btnc1:
